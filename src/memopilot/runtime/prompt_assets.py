...
from memopilot.runtime.persona import MEMOPILOT_IDENTITY, PERSONALITY_RULES

# 原型 Prompt 按原文保留，避免为了行宽改变模型语义。
# ruff: noqa: E501
...
1. 先调 `recall_memory`（语义层）：query 写成陈述句，如”用户在三月完成了 memopilot 重构”
2. 评估结果：
...
def build_passive_system_prompt(
    *,
    ...
    parts = [
        ...
        build_agent_behavior_rules_prompt(workspace=workspace),
        build_agent_session_context_prompt(channel=channel, chat_id=chat_id),
        build_current_message_time_envelope(message_timestamp=received_at),
    ]
    return "\n\n---\n\n".join(part for part in parts if part.strip())
