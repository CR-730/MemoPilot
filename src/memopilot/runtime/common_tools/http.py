...
class HttpRequester:
    ...
    def _backoff_seconds(self, attempt: int) -> float:
        ...
        return cast(float, max(0.0, delay + random.uniform(-jitter, jitter)))
...
def configure_default_shared_http_resources(resources: SharedHttpResources) -> None:
...
def get_default_http_requester(profile: HttpProfile) -> HttpRequester:
    ...
    return cast(HttpRequester, getattr(resources, profile))
