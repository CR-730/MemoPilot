"""从旧项目直接迁移的 Textual CLI，仅适配 MemoPilot 的本地 TCP 端点。"""
...
from memopilot.channels.cli import DEFAULT_SOCKET, _parse_tcp_endpoint
...
class HeaderBar(Horizontal):
    def compose(self) -> ComposeResult:
        yield Static("# memopilot Agent CLI / Textual", id="title")
        yield Static("0 sent  0 recv  0 tools  connected: no", id="meta")


class FooterBar(Horizontal):
    def compose(self) -> ComposeResult:
        yield Static("memopilot (Textual TUI)", id="footer-left")
        yield Static(
...
class CLITextualApp(App[None]):
    ...
    async def _connect_and_receive(self) -> None:
        try:
            endpoint = _parse_tcp_endpoint(self.socket_path)
            if endpoint is None:
            ...
            self._write_system_message("请先启动主进程: uv run python main.py")
            self.connected = False
            self._refresh_header()
            return
        except Exception as exc:
    ...
    def _refresh_header(self) -> None:
        ...
        self.query_one("#meta", Static).update(
            f"{self.stats.sent} sent  {self.stats.received} recv  "
            f"{self.stats.tool_calls} tools  connected: {state}"
        )
    ...
    def _write_tool_chain(self, tool_chain: list[dict[str, object]]) -> None:
        log = self.query_one(RichLog)
...
async def run_tui_async(socket_path: str = DEFAULT_SOCKET) -> None:
    mouse_enabled = _env_bool("MEMOPILOT_TUI_MOUSE", True)
    await CLITextualApp(socket_path).run_async(mouse=mouse_enabled)


__all__ = ["CLITextualApp", "run_tui_async"]
